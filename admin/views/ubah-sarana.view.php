                ...
                <tbody>
                    <?php foreach ($getdata as $i => $a) : ?>
                        <tr>
                            ...
                            <td><a class="text-dark text-decoration-none" href="detail-sarana?id=<?= $a["id"] ?>"><?= $a['nama_sarana']; ?></a></td>
                            <td><?= $a['file_json']; ?></td>
                            <td class="text-center">
                                <a class="text-decoration-none" href="form-update-sarana?id=<?= $a["id"] ?>">
                                    <span class="badge bdg-a text-bg-warning p-2"><i class="fa-regular fa-pen-to-square"></i>Ubah</span>
                                </a>
                                <span class="fw-bold spase">|</span>
                                <a href="javascript:void(0);" class="hapusData" id_sarana="<?= $a["id"] ?>">
...
<script>
    ...
    //         if (konfirmasi) {
    //             console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_sarana') + " telah dihapus!");
    //             // Tambahkan tindakan penghapusan di sini (misalnya, hapus data dari server)
    //             window.location.href = "functions/delete-sarana.php?id=" + hapusButtons[i].getAttribute(
    //                 'id_sarana');
    //         } else {
        ...
        hapusButtons[i].addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_sarana') +
                        " telah dihapus!");

                    // Perform deletion action (e.g., delete data from the server)
                    window.location.href = "functions/delete-sarana.php?id=" + hapusButtons[i]
                        .getAttribute(
                            'id_sarana');
                } else {
                    console.log("Penghapusan dibatalkan.");
