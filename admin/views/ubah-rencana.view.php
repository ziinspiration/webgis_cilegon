                ...
                <tbody>
                    <?php foreach ($getdata as $i => $a) : ?>
                        <tr>
                            ...
                            <td><a class="text-dark text-decoration-none" href="detail-rencana?id=<?= $a["id"] ?>"><?= $a['nama_rencana']; ?></a></td>
                            <td><?= $a['file_json']; ?></td>
                            <td class="text-center">
                                <a class="text-decoration-none" href="form-update-rencana?id=<?= $a["id"] ?>">
                                    <span class="badge bdg-a text-bg-warning p-2"><i class="fa-regular fa-pen-to-square"></i>Ubah</span>
                                </a>
                                <span class="fw-bold spase">|</span>
                                <a href="javascript:void(0);" class="hapusData" id_rencana="<?= $a["id"] ?>">
...
<script>
    ...
    keyword.onkeyup = function() {
        fetch("ajax/search-update-rencana.php?keyword=" + keyword.value)
    ...
    //         if (konfirmasi) {
    //             console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_rencana') + " telah dihapus!");
    //             // Tambahkan tindakan penghapusan di sini (misalnya, hapus data dari server)
    //             window.location.href = "functions/delete-rencana.php?id=" + hapusButtons[i].getAttribute(
    //                 'id_rencana');
    //         } else {
        ...
        hapusButtons[i].addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_rencana') +
                        " telah dihapus!");

                    // Perform deletion action (e.g., delete data from the server)
                    window.location.href = "functions/delete-rencana.php?id=" + hapusButtons[i]
                        .getAttribute(
                            'id_rencana');
                } else {
                    console.log("Penghapusan dibatalkan.");
