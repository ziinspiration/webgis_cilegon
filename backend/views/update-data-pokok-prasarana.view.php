...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE data_prasarana SET nama_data='$nama_data' WHERE id='$id'";
...
<div class="container-fluid">
    <h1 class="mt-5 mb-2 text-center text-dark">Update data pokok prasarana</h1>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" action="">
                                ...
                                <input class="bg-transparent border-0 px-1 <?= empty($message) ? 'editable' : ''; ?>"
                                    type="text" name="nama_data" value="<?= $a['nama_data']; ?>"
                                    <?php if (!empty($message)) echo 'readonly'; ?>>
                        </td>
                        <td>
                            <?php if (empty($message)) : ?>
                            <a class="text-warning me-2 edit-icon" href="#"><i
                                    class="fa-solid fa-pen-to-square"></i></a>
                            <button type="submit" class="text-primary btn save-icon" name="save"><i
                                    class="fa-solid fa-floppy-disk"></i></button>
                            <?php endif; ?>
                            <a href="javascript:void(0);" class="hapusData text-danger ms-2"
                                id_pokok="<?= $a["id"] ?>"><i class=" fa-solid fa-trash"></i></a>
                            </form>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 'update-data-pokok-prasarana'; // Ganti dengan URL halaman tujuan
        }
    });
    <?php else : ?>
    Swal.fire({
    ...
    hapusButtons[i].addEventListener('click', function(event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "functions/delete-pokok-prasarana.php?id=" + hapusButtons[i]
                    .getAttribute(
                        'id_pokok');
            } else {
