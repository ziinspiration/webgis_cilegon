                    ...
                    <?php foreach ($getdata as $i => $a) : ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <a class="text-decoration-none" href="form-update-publikasi?id=<?= $a["id"] ?>">
                                    <span class="badge bdg-a text-bg-warning p-2"><i class="fa-regular fa-pen-to-square"></i>Ubah</span>
                                </a>
                                <span class="fw-bold spase">|</span>
                                <a href="javascript:void(0);" class="hapusData" id_publikasi="<?= $a["id"] ?>">
...
<script>
    ...
    keyword.onkeyup = function() {
        fetch("ajax/search-update-publikasi.php?keyword=" + keyword.value)
        ...
        hapusButtons[i].addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_publikasi') +
                        " telah dihapus!");

                    // Perform deletion action (e.g., delete data from the server)
                    window.location.href = "functions/delete-publikasi.php?id=" + hapusButtons[i]
                        .getAttribute(
                            'id_publikasi');
                } else {
