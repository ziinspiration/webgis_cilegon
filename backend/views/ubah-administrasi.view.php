                ...
                <tbody>
                    <?php foreach ($getdata as $i => $a) : ?>
                        <tr>
                            ...
                            <td><a class="text-dark text-decoration-none" href="detail-administrasi?id=<?= $a["id"] ?>"><?= $a['nama_adm']; ?></a></td>
                            <td><?= $a['file_json']; ?></td>
                            <td class="text-center">
                                ...
                                <a href="javascript:void(0);" class="hapusData" id_administrasi="<?= $a["id"] ?>">
...
<script>
    ...
    keyword.onkeyup = function() {
        fetch("ajax/search-update-administrasi.php?keyword=" + keyword.value)
        ...
        hapusButtons[i].addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("Data dengan ID " + hapusButtons[i].getAttribute('id_administrasi') +
                        " telah dihapus!");

                    // Perform deletion action (e.g., delete data from the server)
                    window.location.href = "functions/delete-administrasi.php?id=" + hapusButtons[i]
                        .getAttribute(
                            'id_administrasi');
                } else {
